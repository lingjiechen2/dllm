...
class ScriptArguments:
    ...
    seed: int = 42
    limit: int = 300000  # for debugging
...
def bert_predict(context, beam_width=128, max_predictions=6):
    ...
    Returns both the decoded string and the list of predicted tokens.
    """
    generated_tokens = []
    generated_token_ids = []
    # breakpoint()
...
def predict_wrapper(line):
    """
    Wrapper for multi-step BERT generation.
    Re-tokenizes the generated continuation with BasicTokenizer
    and extracts the first valid predicted word.
    """
    ...
    try:
        ...
        predicted_text, predicted_tokens = bert_predict(context, beam_width=128, max_predictions=6)
        # breakpoint()
        # Only take the newly generated portion beyond the input context
        predicted_part = predicted_text.strip()

        # Use BasicTokenizer for robust, language-aware splitting
        ...
        print(f"predicted_text raw repr: {repr(predicted_text) if 'predicted_text' in locals() else '<N/A>'}")
        predicted_word = ""

    print(f"generated content: {predicted_text}, prediction:{predicted_word[:len(last_word)]}, ground_truth:{last_word}")
...
for i, line in enumerate(tqdm(lines, desc="Evaluating BERT on LAMBADA", unit="line")):
    ...
    is_error = predicted_word[:len(last_word)].lower() != last_word.lower()
...
print(f"{script_args.model_name_or_path}")
