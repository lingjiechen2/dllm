...
def preprocess(text):
    # Normalize smart quotes, dashes, and ellipses to ASCII
    text = text.replace("“", '"')
    text = text.replace("”", '"')
    text = text.replace("''", '"')
    text = text.replace("``", '"')
...
def predict(line, max_predictions):
    ...
    try:
        ...
        print(f"KeyError: {e}")
        print("Problematic line (raw):")
        print(repr(line))
        print("Character ordinals in line:")
        print([ord(ch) for ch in line])
        print("Problematic characters (non-ASCII only):")
        bad_chars = [ch for ch in line if ord(ch) > 127]
        print(bad_chars)
        print("=" * 80)
        breakpoint()
        raise  # re-raise after logging so you can inspect in debugger
...
def main():
    ...
    print(f"Using preprocess {args.preprocess}")
    if args.preprocess:
