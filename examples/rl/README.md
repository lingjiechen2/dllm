...
└── a2d/
    └── mdlm/
        └── train.py                # GRPO training for A2D (MDLM)
```
...
For example, to run GRPO on [`LLaDA-8B-Instruct`](https://huggingface.co/GSAI-ML/LLaDA-8B-Instruct) with `gsm8k` on 1 GPU:tell 
```shell
accelerate launch \
...
For example, to run GRPO on [`Qwen3-0.6B-diffusion-mdlm-v0.1`](https://huggingface.co/dllm-hub/Qwen3-0.6B-diffusion-mdlm-v0.1) with `gsm8k` on 1 GPU:
```shell
accelerate launch \
    --config_file scripts/accelerate_configs/ddp.yaml --num_processes 1 \
